...
INSTRUCTIONS FOR INSTALLATION: (Version 3.0.14 - February 2026)
-----------------------------

The software is available in gzipped tar format:

	LKH-3.0.14.tgz	(approximately 2.3 MB)

Download the software and execute the following UNIX commands:

  	tar xvfz LKH-3.0.14.tgz
   	cd LKH-3.0.14
	make

An executable file called LKH will now be available in the directory LKH-3.0.14.
...
CHANGES IN VERSION 3.0.14:
-------------------------

Added code for solving

the Orienteering Problem (OP),
the Traveling Salesman with Multiple Drones (TSP-mD) 
(PROBLEM_TYPE = TSPMD; new keywords: DRONES, ENDURANCE),
the Open k-Traveling Salesman Problem (O-k-TSP)
(PROBLEM_TYPE = OKSTP), and
the Profitable Tour Problem (PTP); new keyword: ALPHA
...
the precedence-constrained colored traveling salesman problem (PCTSP),
and the constrained asymmetric traveling salesman path problem (CATSPP).
