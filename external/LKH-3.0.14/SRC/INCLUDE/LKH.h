...
// #define PenaltyAware (CurrentPenalty != 0)
#define PenaltyAware (CurrentPenalty != 0 &&\
                      ProblemType != GCTSP &&\
                      ProblemType != MSCTSP &&\
                      ProblemType != PCTSP)
...
enum Types { TSP, ATSP, SOP, TSPTW, HCP, CCVRP, CVRP, ACVRP, CVRPTW, RCTVRP, 
    ...
    CTSP, STTSP, ADCVRP, KTSP, CBTSP, CBnTSP, PTSP, CTSP_D, CluVRP, SoftCluVRP,
    GCTSP, CCCTSP, MSCTSP, PCTSP, TSPMD, OP, OKTSP, PTP
};
...
enum InitialTourAlgorithms { BORUVKA, CTSP_ALG, CVRP_ALG, GCTSP_ALG,
    GREEDY, MOORE, MTSP_ALG, NEAREST_NEIGHBOR, QUICK_BORUVKA, SIERPINSKI,
    SOP_ALG, TSPDL_ALG, WALK, PCTSP_ALG
};
...
struct Node {
    ...
    int OriginalId;    /* The original Id in a STTSPinstance */
    ...
    int Score;          /* The node score for an OP or PTP instance */
...
extern double Alpha;           /* Travel cost per unit of distance in
                                  a PTP instance */
extern int AscentCandidates;   /* Number of candidate edges to be associated
                                  with each node during the ascent */
extern int Asymmetric;         /* Is the instance asymmetric? */
...
extern int *BetterTour;        /* Table containing the currently best tour 
                                  in a run */
extern int BWTSP_B;            /* Number of black nodes in a BWTSP instance */ 
extern int BWTSP_Q;            /* Maximum number of subsequent white nodes in a
                                  BWTSP instance */ 
extern int BWTSP_L;            /* Maximum length of any path between two black
...
extern double CostLimit;       /* Maximum cost for an OP instance */
...
extern int DemandDimension;    /* Number of commodities in a M-PDTSP instance */
extern Node *Depot;
...
extern Node *FirstActive, *LastActive; /* First and last node in the list 
                                          of "active" nodes */
extern Constraint *FirstConstraint; /* First constraint in the list of SOP 
...
extern int GVRPSets;   /* Number of GVRP sets */
...
extern int k;          /* The k-value for solving a k-TSP/O-k-tsp  instance */
...
extern int Probability          ; /* The probability for PTSP (in percent) */
...
extern int RelaxationLevel;      /* d-relaxation level for TSP-D */
...
extern int Scale;      /* Scale factor for Euclidean and ATT instances */ 
...
extern int SubsequentMoveTypeSpecial; /* A special (3- or 5-opt) subsequent move
                                        is used */
...
extern int IsChild;    /* Is the current tour a child tour */
extern GainType TSPTW_CurrentMakespanCost;
extern int TSPTW_Makespan;
extern int *ColorCount;
extern short OptimizePenalty;
extern int RouteNodes;
extern GainType RouteCost;
extern GainType RouteScore;
...
extern int CandidateSetSymmetric, CandidateSetType, Capacity,
           CoordType, DelaunayPartitioning, DelaunayPure,
           Drones, ExternalSalesmen,
...
GainType Penalty_CluVRP(void);
GainType Penalty_CBTSP(void);
GainType Penalty_CBnTSP(void);
