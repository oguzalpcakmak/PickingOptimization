...
{
    GainType Cost, OldOptimum, BestRouteScore, BestRouteCost;
    double Time, LastTime;
    Node *N;
    int BestRouteNodes = 0, i;
    ...
    if (Norm != 0 || Penalty) {
        ...
        BestRouteScore = PLUS_INFINITY;
        BestRouteCost = PLUS_INFINITY;
    } else {
    ...
    if (ProblemType == ACVRP ||
        ...
        SOP_Report(BestCost);
        if (BestRouteNodes > 0) {
            int DecimalPlaces = Scale == 1 ? 0 : log10(Scale);
            printff("  Route nodes = %d\n", BestRouteNodes);
            if (ProblemType == OP || ProblemType == PTP)
                printff("  Route score = " GainFormat "\n", -BestRouteScore);
            printff("  Route cost = %.*lf\n", 
                    DecimalPlaces,
                    (double) BestRouteCost / Scale);
        }
    }
    printff("\n");
    return EXIT_SUCCESS;
