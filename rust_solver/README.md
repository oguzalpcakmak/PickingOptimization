...
- `--fallback-method visited-area`: v2 completion rule, prioritizing visited box, half-block, aisle, floor, then random.
...
This writes outputs under `outputs/benchmark_outputs/rust_current_best_benchmark/` and the report to `reports/benchmarks/RUST_CURRENT_BEST_BENCHMARK.md`.

To benchmark the v2 fallback method:

```bash
.venv/bin/python src/benchmark_rust_current_best.py --fallback-method visited-area
```
