"""GRASP-style multi-start heuristic variant without final 2-opt cleanup.

This solver keeps the same construction logic as grasp_heuristic.py:
  1. randomized article selection from a top-priority prefix,
  2. randomized location selection from a restricted candidate list,
  3. best-solution retention across multiple starts.

The only intentional difference is the route rebuild stage:
  - floor routes are rebuilt without the final 2-opt polishing pass.

This makes it useful for benchmarking how much the final 2-opt cleanup is
actually contributing on the current instances.
"""
...
from grasp_heuristic import construct_once
    ...
    *,
    ...
    while completed_iterations < max_iterations:
        ...
        candidate_solution = build_solution(
            algorithm="GRASP Multi-Start Heuristic (No 2-opt)",
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="GRASP multi-start heuristic without final 2-opt cleanup.")
    ...
    parser.add_argument(
        ...
        default="PickDataOutput_GRASP_No2Opt.csv",
        help="Pick CSV output path.",
    )
    parser.add_argument(
        "--alternative-locations-output",
        default="AlternativeLocationsOutput_GRASP_No2Opt.csv",
        help="Alternative locations CSV output path. Pass empty string to disable.",
    )
    args = parser.parse_args(argv)

    print("╔════════════════════════════════════════════════════════════════════╗")
    print("║  GRASP MULTI-START HEURISTIC (NO 2-OPT)                          ║")
    print("╚════════════════════════════════════════════════════════════════════╝")
    print()
