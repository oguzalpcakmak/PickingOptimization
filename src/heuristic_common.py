...
def _route_edge_cost(left: Node2D | None, right: Node2D | None) -> float:
    if left is None and right is None:
        return 0.0
    if left is None:
        return entry_exit_distance(right)  # type: ignore[arg-type]
    if right is None:
        return entry_exit_distance(left)
    return same_floor_distance(left, right)


def _node_at(route: Sequence[Node2D], index: int) -> Node2D | None:
    if index < 0 or index >= len(route):
        return None
    return route[index]


def _node_at_after_swap(route: Sequence[Node2D], index: int, left: int, right: int) -> Node2D | None:
    if index < 0 or index >= len(route):
        return None
    if index == left:
        return route[right]
...
def two_opt_delta(route: Sequence[Node2D], left: int, right: int) -> float:
    prev_node = _node_at(route, left - 1)
    next_node = _node_at(route, right + 1)
    old_cost = _route_edge_cost(prev_node, route[left]) + _route_edge_cost(route[right], next_node)
    new_cost = _route_edge_cost(prev_node, route[right]) + _route_edge_cost(route[left], next_node)
    return new_cost - old_cost


def swap_delta(route: Sequence[Node2D], left: int, right: int) -> float:
    n = len(route)
    affected_edges = {left - 1, left, right - 1, right}
    old_cost = 0.0
    new_cost = 0.0
    for edge_left in affected_edges:
        if edge_left < -1 or edge_left >= n:
            continue
        old_cost += _route_edge_cost(_node_at(route, edge_left), _node_at(route, edge_left + 1))
        new_cost += _route_edge_cost(
            _node_at_after_swap(route, edge_left, left, right),
            _node_at_after_swap(route, edge_left + 1, left, right),
        )
    return new_cost - old_cost


def relocate_delta(route: Sequence[Node2D], source: int, target: int) -> float:
    node = route[source]
    prev_node = _node_at(route, source - 1)
    next_node = _node_at(route, source + 1)
    removal_delta = _route_edge_cost(prev_node, next_node) - _route_edge_cost(prev_node, node) - _route_edge_cost(node, next_node)

    without = route[:source] + route[source + 1 :]
    prev_insert = without[target - 1] if target > 0 else None
    next_insert = without[target] if target < len(without) else None
    insert_delta = _route_edge_cost(prev_insert, node) + _route_edge_cost(node, next_insert) - _route_edge_cost(prev_insert, next_insert)
    return removal_delta + insert_delta


def _apply_two_opt_move(route: list[Node2D], left: int, right: int) -> None:
    route[left : right + 1] = reversed(route[left : right + 1])


def _apply_swap_move(route: list[Node2D], left: int, right: int) -> None:
    route[left], route[right] = route[right], route[left]


def _apply_relocate_move(route: list[Node2D], source: int, target: int) -> None:
    node = route.pop(source)
    route.insert(target, node)


def cleanup_route_delta(
    route: Sequence[Node2D],
    *,
    operator: str = "2-opt",
    strategy: str = "best",
    max_passes: int = 3,
) -> list[Node2D]:
    """Clean up a route with delta-cost local search.

    ``strategy='first'`` applies the first improving move found in scan order.
    ``strategy='best'`` scans the whole neighborhood and applies the best move.
    """

    normalized_operator = operator.lower().replace("_", "-")
    if normalized_operator in {"none", "no-op", "noop"}:
        return list(route)
    if normalized_operator in {"two-opt", "2-opt", "2opt"}:
        normalized_operator = "2-opt"
    if normalized_operator not in {"2-opt", "swap", "relocate"}:
        raise DataError(f"Unsupported route cleanup operator '{operator}'.")
    if strategy not in {"first", "best"}:
        raise DataError(f"Unsupported route cleanup strategy '{strategy}'.")

    improved_route = list(route)
    if len(improved_route) <= 1 or (normalized_operator == "2-opt" and len(improved_route) <= 2):
    ...
    for _ in range(max(0, max_passes)):
        ...
        if normalized_operator == "2-opt":
            for left in range(len(improved_route) - 1):
                for right in range(left + 1, len(improved_route)):
                    delta = two_opt_delta(improved_route, left, right)
                    if delta < -1e-9:
                        if strategy == "first":
                            _apply_two_opt_move(improved_route, left, right)
                            applied = True
                            break
                ...
                if applied:
                    break
            if strategy == "best" and best_move is not None:
                _, left, right = best_move
                _apply_two_opt_move(improved_route, left, right)
                applied = True

        elif normalized_operator == "swap":
            for left in range(len(improved_route) - 1):
                for right in range(left + 1, len(improved_route)):
                    delta = swap_delta(improved_route, left, right)
                    if delta < -1e-9:
                        if strategy == "first":
                            _apply_swap_move(improved_route, left, right)
                            applied = True
                            break
                        if best_move is None or delta < best_move[0] - 1e-12:
                            best_move = (delta, left, right)
            ...
            for source in range(len(improved_route)):
                for target in range(len(improved_route)):
                    if target == source:
                        continue
                    delta = relocate_delta(improved_route, source, target)
                    if delta < -1e-9:
                        if strategy == "first":
                            _apply_relocate_move(improved_route, source, target)
                            applied = True
                            break
                        if best_move is None or delta < best_move[0] - 1e-12:
...
def cleanup_solution_routes(
    ...
    *,
    ...
    phase_times = dict(solution.phase_times)
    phase_times["route_cleanup"] = cleanup_time
    phase_times["total"] = solution.solve_time + cleanup_time

    cleaned_solution = Solution(
        algorithm=f"{solution.algorithm} + {operator} cleanup ({strategy})",
        floor_results=sorted(floor_results, key=lambda result: floor_index(result.floor)),
        total_distance=total_distance,
        total_thms=len(all_thms),
        total_floors=len(floor_results),
        total_picks=sum(len(result.picks) for result in floor_results),
        solve_time=solution.solve_time + cleanup_time,
        phase_times=phase_times,
        objective_value=objective_value,
        demands=dict(solution.demands),
        relevant_locs=list(solution.relevant_locs),
        loc_lookup=dict(solution.loc_lookup),
        notes={
            **solution.notes,
            "route_cleanup": f"{operator} ({strategy})",
            "route_cleanup_operator": operator,
            "route_cleanup_strategy": strategy,
            "route_cleanup_max_passes": max_passes,
            "route_cleanup_time": cleanup_time,
        },
    )
    return cleaned_solution, cleanup_time
...
def _load_lk_runtime():
    ...
    lk_src = repo_root / "external" / "lk_heuristic-master" / "src"
    if not lk_src.exists():
    ...
    try:
        ...
        raise DataError(
            ...
            "Expected either a local 'external/lk_heuristic-master/src' checkout or an installed "
