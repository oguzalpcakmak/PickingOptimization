"""Comprehensive benchmark runner for the C++ current-best solver."""
...
def _int_note(notes: dict[str, Any], key: str) -> int:
    try:
...
def run_case(
    *,
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"C++ solver failed for {dataset['slug']} / {budget['slug']} / {cleanup_slug} / {strategy_slug}. See {log_output}.")
    ...
    row = {
        ...
        "remaining_units_before_fallback": _int_note(notes, "remaining_units_before_fallback"),
        "strict_steps": _int_note(notes, "strict_steps"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark the C++ current-best solver.")
    parser.add_argument("--output-dir", default="outputs/benchmark_outputs/cpp_current_best_benchmark")
    parser.add_argument("--report", default="reports/benchmarks/CPP_CURRENT_BEST_BENCHMARK.md")
            ...
            for cleanup_slug, cleanup_operator in CLEANUPS:
                ...
                for strategy_slug, strategy_label in STRATEGIES:
                    if selected_strategies and strategy_slug not in selected_strategies:
                        continue
                    print(
                        f"Running C++ {dataset['slug']} / {budget['slug']} / {cleanup_slug} / {strategy_slug} ...",
                        flush=True,
                    )
                    rows.append(
                        run_case(
