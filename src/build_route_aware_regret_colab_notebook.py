...
OUTPUT = REPO_ROOT / "notebooks" / "route_aware_regret_colab.ipynb"
...
def main() -> None:
    ...
    regret_solver = (ROOT / "regret_based_heuristic.py").read_text(encoding="utf-8")

    intro = textwrap.dedent(
        """
        # Route-Aware Regret Heuristic for Google Colab

        This notebook is a self-contained Colab wrapper for the repository's `route-aware regret` solver.
        ...
        - runs the route-aware regret heuristic with configurable weights and filters,
    ...
    config = textwrap.dedent(
        ...
        PICK_OUTPUT = "PickDataOutput_RegretHeuristic.csv"
        ALT_OUTPUT = "AlternativeLocationsOutput_RegretHeuristic.csv"
        """
    ).strip() + "\n"
    ...
    solve = textwrap.dedent(
        ...
        from regret_based_heuristic import solve
    ...
    notes = textwrap.dedent(
        ...
        - To rerun with different weights or article filters, edit the configuration cell and rerun the input resolution, solve, preview, and download cells.
    ...
    notebook = {
        "cells": [
            ...
            writefile_cell("regret_based_heuristic.py", regret_solver),
            code_cell(config),
